...
# --- 2. GENERATORE PROMPT (LUSSO & BUSINESS) ---
...
def genera_coaching(row):
    ...
    intro = random.choice([
        "🚀 𝗠𝗶𝗻𝗱𝘀𝗲𝘁 𝗜𝗺𝗺𝗼𝗯𝗶𝗹𝗶𝗮𝗿𝗲:", "💡 𝗖𝗼𝗻𝘀𝗶𝗴𝗹𝗶𝗼 𝗱𝗲𝗹 𝗴𝗶𝗼𝗿𝗻𝗼:",
        "🏠 𝗩𝗶𝘀𝗶𝗼𝗻𝗲 𝗲 𝗦𝘂𝗰𝗰𝗲𝘀𝘀𝗼:", "🔥 𝗣𝗲𝗿 𝗶 𝘃𝗲𝗿𝗶 𝗹𝗲𝗮𝗱𝗲𝗿:"
    ])
    
    msg = ""
    if "motiva" in cat:
        msg = "Non aspettare il momento giusto, crealo. Vince chi ha fame."
    elif "vendita" in cat or "business" in cat:
        msg = "La vendita non è convincere, è aiutare il cliente a decidere. Ogni 'no' ti avvicina al 'sì'."
...
# --- 8. FUNZIONI SOCIAL (CON AUTENTICAZIONE PAGINA) ---
...
def get_page_access_token(user_token, page_id):
    """Scambia il token utente con il token della pagina specifica"""
    try:
        ...
        if resp.status_code == 200:
            ...
            for page in data:
                if page.get('id') == str(page_id):
                    print(f"✅ Trovata Pagina: {page.get('name')}")
                    return page.get('access_token')
            print(f"❌ Pagina con ID {page_id} non trovata. Controlla che l'utente sia Admin.")
        else:
            print(f"❌ Errore recupero pagine: {resp.text}")
    except Exception as e:
        print(f"❌ Errore connessione token: {e}")
    return None
...
def post_facebook(img_bytes, message):
    ...
    # 1. Recupera il Token specifico della Pagina "Antonio Giancani"
    page_token = get_page_access_token(FACEBOOK_TOKEN, TARGET_PAGE_ID)
    
    if not page_token:
        print("❌ Impossibile pubblicare: Fallito recupero Page Token.")
        return

    # 2. Pubblica usando il Token della Pagina
    url = f"https://graph.facebook.com/v19.0/{TARGET_PAGE_ID}/photos?access_token={page_token}"
...
if __name__ == "__main__":
    ...
    if row is not None:
        ...
        caption = (
            ...
            f"#immobiliare #mindset #successo #realestate #business #antoniogiancani #motivazione"
        )
